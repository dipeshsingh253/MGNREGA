...
public class GramPanchayatMemberImpl implements GramPanchayatMemberFunction {
	...
	public GramPanchayatMember loginGpm(String username, String password) {
		...
		try (Connection connection = DButility.provideConnection()) {
			...
			if (rs.next()) {
				...
				gpm = new GramPanchayatMember(id, name, email, username, password, proj_id);
	...
	public void employeeDetails(int eid) {
		...
		try (Connection connection = DButility.provideConnection()) {
			...
			if (!rs.next()) {
				throw new GramPanchayatMemberException("No Employee Related to this Employee Id is Availible");
			}
	...
	public void assignProjecttoEmployee(int... args) {
		...
		try (Connection connection = DButility.provideConnection()) {
			...
			PreparedStatement ps = connection.prepareStatement(
					"update Employees set proj_id = ?, wage_perday = ?, work_days = ? where emp_id = ?");
	...
	public void dayAndWagesOfEmployee() {
		try (Connection connection = DButility.provideConnection()) {
			...
			if (!rs.next()) {
				throw new GramPanchayatMemberException("No Employees Availible");
			} else {
